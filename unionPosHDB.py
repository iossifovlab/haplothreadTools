...
print("read POS", len(POS), "lines", file=sys.stderr)
...
for i,f in enumerate(posFiles[1:]):
    print("processing file", i, time() -st, file=sys.stderr)
    POS1 = np.genfromtxt(f,delimiter='\t',dtype=dt,names=True, case_sensitive=True)
    print("read POS1", len(POS1), "lines", file=sys.stderr)
    ...
    print('read family ', f, (time() - st), file=sys.stderr) 
