...
statsP = np.array([{i:cnt[v] for i,v in x.items()} for cnt, x in zip(statsPP, ALLMap)])
statsC = np.array([{i:cnt[v] for i,v in x.items()} for cnt, x in zip(statsCC, ALLMap)])

print >>sys.stderr, "Read", summaryFn
